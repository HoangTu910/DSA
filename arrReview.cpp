...
void createHeap(int a[], int n=SIZE){
    int start = (n-1)/2;
    while(start >= 0){
        Shift(a, start, n-1);
        start--;
...
void heapSort(int a[], int n=SIZE){
    createHeap(a, n);
    int end = n - 1;
    while(end > 0){
        swap(a[0], a[end]);
...
void printArr(int *a, int n=SIZE){
    for(int i = 0; i < n; i++){
        cout << a[i] << " ";
    }
    cout << endl;
...
//Radix Sort
void radixSort(int a[], int n=SIZE){
    int output[100], maxValue = 0, exp = 1;
    for(int i = 0; i < n; i++){
        if(maxValue < a[i]){
            maxValue = a[i];
        }
...
//Merge Sort
void Merge(int a[], int n, int nb, int nc, int k, int b[], int c[]){
...
void MergeSort(int a[], int n=SIZE){
    int b[100], c[100], k = 1;
    int pb, pc;
    do{
        Distribute(a,n,pb,pc,k,b,c);
        Merge(a,n,pb,pc,k,b,c);
        k *= 2;
    } while (k < n);
}

int main(){
    int a[SIZE] = {732,411,545,812,1421,22,631,90,863,0};
    unsigned int h[3] = {5,3,1};
    int k = 3;
    printArr(a);
    //heapSort(a);
    //shellSort(a, SIZE, h, k);
    //radixSort(a);
    MergeSort(a);
    printArr(a);
}
